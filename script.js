const player1 = "X";
const player2 = "O";
var playerTime = player1;
var gameOver = false;

var pontos1 = 0;
var pontos2 = 0;
var pontosVelha = 0;

atualizarOmostrador();
inicializarEspacos();
verificarVencedor();

function atualizarOmostrador() {


  if (gameOver) {
    return;
  }

  if (playerTime == player1) {

    var player = document.querySelectorAll("div#mostrador img")[0];
    player.setAttribute("src", "x.png");
  } else {

    var player = document.querySelectorAll("div#mostrador img")[0];
    player.setAttribute("src", "o.png");
  }
}

function inicializarEspacos() {

  var espacos = document.getElementsByClassName("espaco");
  for (var i = 0; i < espacos.length; i++) {
    espacos[i].addEventListener("click", function () {
      if (gameOver) {
        return;
      }

      if (this.getElementsByTagName("img").length == 0) {
        if (playerTime == player1) {
          this.innerHTML = "<img src='x.png' class='aa'>";
          this.setAttribute("jogada", player1);
          playerTime = player2;
        }
        else {
          this.innerHTML = "<img src='o.png' class='bb'>";
          this.setAttribute("jogada", player2);
          playerTime = player1;
        }

        atualizarOmostrador();
        verificarVencedor();
      }
    });
  }
}

async function verificarVencedor() {

  var a1 = document.getElementById("a1").getAttribute("jogada");
  var a2 = document.getElementById("a2").getAttribute("jogada");
  var a3 = document.getElementById("a3").getAttribute("jogada");

  var b1 = document.getElementById("b1").getAttribute("jogada");
  var b2 = document.getElementById("b2").getAttribute("jogada");
  var b3 = document.getElementById("b3").getAttribute("jogada");

  var c1 = document.getElementById("c1").getAttribute("jogada");
  var c2 = document.getElementById("c2").getAttribute("jogada");
  var c3 = document.getElementById("c3").getAttribute("jogada");

  var vencedor = "";

  if ((a1 == a2 && a1 == a3) && (a1 != "")) {
    vencedor = a1;

  } else if ((b1 == b2 && b1 == b3) && (b1 != "")) {
    vencedor = b1;

  } else if ((c1 == c2 && c1 == c3) && (c1 != "")) {
    vencedor = c1;

  } else if ((a1 == b1 && a1 == c1) && (a1 != "")) {
    vencedor = a1;

  } else if ((a2 == b2 && a2 == c2) && (a2 != "")) {
    vencedor = a2;

  } else if ((a3 == b3 && a3 == c3) && (a3 != "")) {
    vencedor = a3;

  } else if ((a1 == b2 && a1 == c3) && (a1 != "")) {
    vencedor = a1;

  } else if ((a3 == b2 && a3 == c1) && (a3 != "")) {
    vencedor = a3;
  }

  if (vencedor != "") {
    gameOver = true;

    if (vencedor == "X") {
      document.getElementById("mostravencedor").innerHTML = "Player 1 venceu! ";
      pontos1 = pontos1 + 1;
      document.getElementById("pontos1").innerHTML = pontos1;
    } else {
      pontos2 = pontos2 + 1;
      document.getElementById("mostravencedor").innerHTML = "Player 2 venceu! ";
      document.getElementById("pontos2").innerHTML = pontos2;
    }

  } else if ((a1 != "" && a2 != "" && a3 != "" && b1 != "" && b2 != "" && b3 != "" && c1 != "" && c2 != "" && c3 != "" && vencedor == "")) {

    pontosVelha = pontosVelha + 1;
    document.getElementById("mostravencedor").innerHTML = "Deu Velha! "
    document.getElementById("pontos-velha").innerHTML = pontosVelha;
    gameOver = true;
  }
}

let espacos = document.getElementsByClassName("espaco");
let reinicia = document.getElementById("botaoreinicia");
reinicia.addEventListener("click", function () {

  for (let i = 0; i < espacos.length; i++) {
    if (gameOver == true) {
      espacos[i].innerHTML = "";
      espacos[i].setAttribute("jogada", '');
      document.getElementById("mostravencedor").innerHTML = "Divirtam-se!";
    }
  }

  gameOver = false;

})

